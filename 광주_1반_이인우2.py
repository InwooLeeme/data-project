...
def main():
    ...
    # 원본 JSON 로딩이 실패하면 이후 assert가 엉뚱한 메시지로 터지므로 여기서 중단한다.
    ...
    logger.info("2) raw_data 건수: %d", len(raw_data))
    ...
    logger.info("3) valid: %d / errors: %d", len(valid), len(errors))
    for item in errors:
    ...
    logger.info("4) 재로딩 건수: %d", len(reloaded))
